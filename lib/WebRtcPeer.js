 ...
 * limitations under the License.
  * July 2022 this Fork used for Testing ionic capcitor with this by Efficient Apps Inc.
 * forked from latest version july 2023
 */

var freeice = require('freeice')
var inherits = require('inherits')
var UAParser = require('ua-parser-js')
var uuidv4 = require('uuid/v9')
...
function WebRtcPeer(mode, options, callback) {
  ...
  /**
  * Aug 2022 added by efficient apps to add audioSource option for setSinkId
 */
  ...
  if (!pc.getLocalStreams && pc.getSenders) {
    pc.getLocalStreams = function () {
      ...
      pc.getSenders().forEach(function (sender) {
        ...
        catch(error){ logger.error('pc.getSenders stream.addtrack',error)}
      });
      return [stream];
    };
  }
    ...
    pc.getRemoteStreams = function () {
      ...
      pc.getReceivers().forEach(function (sender) {
        ...
        stream.addTrack(sender.track);
           ...
           catch(error){ logger.error('pc.getReceivers stream.addtrack',error)}
      });
      return [stream];
    };
  }
...
/**
 * Added by efficient apps to allow audio source to be set because setSinkId not working in ionic capacitor
 */
 
    logger.debug('audioSource',audioSource);
    if (audioSource && localVideo) localVideo.setSinkId(audioSource);
    logger.debug('setsinkid remotevideo');
    if (audioSource && remoteVideo) remoteVideo.setSinkId(audioSource);
...
//july 2022 added by Efficient Apps to fix issue with ionic capacitor where this does not work
// test failed, this does not work on iphone or android
// no idea why getDisplayMedia is not a function when it is supposed to be
  WebRtcPeer.prototype.shareScreen = function (displayMediaOptions) {
...
WebRtcPeer.prototype.dispose = function () {
  ...
  try {
    ...
    if (pc) {
      ...
      // july 2022 pc.getLocalStreams().forEach(streamStop)
